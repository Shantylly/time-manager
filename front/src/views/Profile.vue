...
const idUser = ref(sessionStorage.getItem('userId'))
...
async function deleteAccount() {
  ...
  await instance.delete(`users/${id.value}`)
    ...
    <div class="w-full p-6 bg-white rounded-md shadow-md">
      <div class="flex items-center justify-between border-b-4 border-indigo-600 pb-5">
        <h1 class="text-2xl font-semibold text-gray-700 m-2 flex-1">
          {{ isModifying ? 'UPDATE INFORMATION' : 'PROFILE' }}
        </h1>
        <svg
          class="w-10 h-10"
          viewBox="0 0 512 512"
      ...
      <div class="flex flex-col">
        ...
        <div class="flex m-5 ml-0 mb-0 w-1/5">
          <button
